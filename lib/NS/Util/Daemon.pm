...
NS::Util::Daemon - CLI for daemontools service.

=cut
package NS::Util::Daemon;
...
=head1 SYNOPSIS
 ...
 my $daemon = NS::Util::Daemon->new( name => 'foo', path => '/path/file' );
...
sub new
{
    ...
    my $name = $self{name};
    my $path = NS::Util::Daemon::Path->new( $self{path} )->make();
    ...
    {
        $conf->{command} = NS::Util::Daemon::Path->macro( $command );
    }
    else
    {
