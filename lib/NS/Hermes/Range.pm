package NS::Hermes::Range;

=head1 NAME

NS::Hermes::Range - Extends NS::Hermes::KeySet.

=head1 SYNOPSIS

 use NS::Hermes::Range;

 my $a = NS::Hermes::Range->new()->load( 'foo{00~99},bar{2~9}baz,-/bar/' );
...
sub parse
{
    ...
    my $result = eval { $self->expr } || NS::Hermes::KeySet->new();
    warn sprintf "%s: $@\n", $self->{index} if $@;
    return $result;
}
...
sub range
{
    ...
    return NS::Hermes::KeySet->new->load( $x, defined $y ? $y : $x );
}
