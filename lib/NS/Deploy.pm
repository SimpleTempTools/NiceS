...
sub new 
{
    ...
    my ( $conf, $maint, $code ) = NS::Deploy::Conf->new(
    ...
    bless +{ 
        ...
        code => NS::Deploy::Code->new( $self{code} => $code ),
        lock => NS::Util::ProcLock->new( "$self{lock}/$name" ),
        ctrl => NS::Deploy::Ctrl->new( $name => "$self{ctrl}" ),
...
#
    ...
    if( -e $cache_path )
    {
        $cache  = NS::Deploy::Cache->load( $cache_path, $maint );
